...
function App() {
    ...
    const recordHistory = useCallback((newElements: CADElement[]) => {
    ...
    const handleUpdateElement = (updatedEl: CADElement) => {
        const newEls = elements.map(el => el.id === updatedEl.id ? updatedEl : el);
    ...
    const confirmTextEntry = () => {
        if (textInputValue && textInputPos) {
            ...
            const newEl: CADElement = {
                id: uid, type: 'TEXT', layer: '0', color: '#cad-text',
                start: textInputPos, text: textInputValue, fontSize: 12
            };
        ...
        <div className="flex flex-col h-screen w-screen overflow-hidden bg-cad-bg text-cad-text font-display relative">
                ...
                <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/20 backdrop-blur-[2px]">
                    <div className="bg-cad-panel border border-cad-border p-4 rounded-lg shadow-xl w-80 animate-in zoom-in-95 duration-200">
                        ...
                        <div className="flex justify-end gap-2">
                            <button onClick={() => setShowTextModal(false)} className="px-3 py-1.5 text-xs text-cad-muted hover:bg-cad-border/50 rounded">Cancel</button>
                            <button onClick={confirmTextEntry} className="px-3 py-1.5 text-xs bg-cad-primary text-white rounded hover:bg-cad-primaryHover">Add Text</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {notification && (
                <div className="absolute top-20 left-1/2 -translate-x-1/2 z-50 animate-in slide-in-from-top-4 fade-in duration-300">
                    <div className="bg-cad-panel/90 border border-cad-primary/50 text-cad-text px-4 py-2 rounded-lg shadow-lg shadow-black/20 flex items-center gap-2 backdrop-blur-sm">
                        <span className="material-symbols-outlined text-cad-primary text-[20px]">info</span>
            ...
            <div className="flex flex-1 overflow-hidden relative">
                ...
                <RightPanel 
                    ...
                    onUpdateElement={(el) => { handleUpdateElement(el); commitAction(elements.map(e => e.id === el.id ? el : e)); }} // Immediate history commit for prop changes
